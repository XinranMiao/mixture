...
model {
  ...
  for (i in 1:N) {
    for (k in 1:K) {
      target += log_mix( p1[k],
                    normal_lpdf(mu[i][k] | beta1_intercept[k] + beta1_t[k] * trt[i], sigmas1[k]),
                    normal_lpdf(mu[i][k] | beta0_intercept[k], sigmas0[k])) ;
    ...
    for (k in 1:K) {
      int delta = bernoulli_rng(p1[k]);
        if (delta == 1) {
          ...
          mu_sim[i, k] = normal_rng(beta0_intercept[k], sigmas0[k]);
        }
      //mu_sim[i, k] = normal_rng(beta1_intercept[k] + beta1_t[k] * trt[i], sigmas1[k]);
    }
    ...
    for (i in 1:N) {
      ...
      for (k in 1:K) {
        ...
        if (delta == 1) {
          ...
          mu_c[i, k] = normal_rng(beta0_intercept[k], sigmas0[k]);
        }
